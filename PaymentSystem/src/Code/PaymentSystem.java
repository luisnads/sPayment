...
public class PaymentSystem {
    ...
    private static int getIndexByIDS(String idSin, String[][] employees) {
        int i = 0;
        while(employees[i][0] != null) {
            if(employees[i][7].equals(idSin))
                break;
            i++;
        }
        if(employees[i][0] == null)
    ...
    private static void moveMatrix(String[][][] matrix) {
        for(int k = 0; k < 9; k++) {
            for(int i = 0; i < 100; i++) {
                for(int j = 0; j < 12; j++) {
    ...
    public static void main(String[] args) {
        ...
        int id = 1, index = 0, iSchedule = 0, dayCount = 0, undoCount = 0, redoCount = 0, operation;
        int[] undoOp = new int[10];
        int[] redoOp = new int[10];
        boolean check = true;
        ...
        System.out.println("Insira data que o Sistema está sendo rodado inicialmente (ex.: 04/06): ");
        ...
        while(check) {
            ...
            if(operation > 0 && operation < 8) {
                if(undoCount < 0) {
                    System.out.println("Undo indisponível!");
                }
                else {
                    undoOp[undoCount] = operation;
                    if (undoCount == 9) moveMatrix(undo);
                ...
                case 8:
                    ...
                    if(uCheck == 1) {
                        if(undoCount < 0)
                            ...
                            redoOp[redoCount] = undoOp[undoCount-1];
                            if(redoCount == 9) moveMatrix(redo);
                            copyMatrix(redo, employees, redoCount++);
                            if(redoCount == 10) redoCount--;

                            if (undoOp[undoCount - 1] == 1) {
                                restoreMatrix(employees, undo, (undoCount - 1));
                                index--;
                                id--;
                            } else if (undoOp[undoCount - 1] == 2) {
                                restoreMatrix(employees, undo, (undoCount - 1));
                                index++;
                            } else if (undoOp[undoCount - 1] == 7) {
                                if (dayCount == 0) dayCount = 13;
                                else dayCount--;
                                if (day == 1) {
                                    month--;
                                    if (((month % 2) == 1 && month < 8) || month == 8 || month == 10 || month == 12)
                                ...
                                restoreMatrix(employees, undo, (undoCount - 1));
                            } else restoreMatrix(employees, undo, (undoCount - 1));
                            undoCount--;
                        }

                    }
                    else if(uCheck == 2){
                        if(redoCount < 0) System.out.println("Redo indisponível!");
                        else if(redoOp[redoCount-1] == 1) {
                            restoreMatrix(employees, redo, (redoCount-1));
                            index++;
                            id++;
                        }
                        else if(redoOp[redoCount-1] == 2) {
                            restoreMatrix(employees, redo, (redoCount-1));
                            index--;
                        }
                        else if(redoOp[redoCount-1] == 7) {
                            if(day+1 == 31)
                            {
                                if((((month%2) == 1 && month < 8)|| month == 8 || month == 10 || month == 12))
                                    day++;
                            ...
                            else if(day+1 == 29 && month == 2) {
                                day = 1;
                                month++;
                            }
                            restoreMatrix(employees, redo, (redoCount-1));
                        }
                        else restoreMatrix(employees, redo, (redoCount-1));
                        redoCount--;
                        undoCount++;
                    }
    ...
    private static void serviceTax(Scanner input, String[][] employees) {
        ...
        System.out.print("Insira o ID Sindical do funcionário que terá a taxa de serviço: ");
        String ID = input.nextLine();
        int index = getIndexByIDS(ID, employees);
        if(index == -1)
            System.out.println("Funcionário não encontrado!\n");
        else {
    ...
    private static void setNewInfo(Scanner input, String[][] employees) {
        ...
        if(index == -1)
            ...
            System.out.println("Deseja mudar o nome do funcionário? [1] Sim | [2] Não");
            ...
            System.out.println("Deseja mudar o tipo do funcionário? [1] Sim | [2] Não");
            confirm = Integer.parseInt(input.nextLine());
            if (confirm == 1) {
                getEmployeeType(input, index, employees);
            }

            System.out.println("Deseja mudar se o funcionário pertence ao sindicato? [1] Sim | [2] Não");
            confirm = Integer.parseInt(input.nextLine());
            if (confirm == 1) {
            ...
            System.out.println("Deseja mudar o endereço do funcionário? [1] Sim | [2] Não");
            confirm = Integer.parseInt(input.nextLine());
            if (confirm == 1) {
            ...
            System.out.println("Deseja mudar o método de pagamento do funcionário? [1] Sim | [2] Não");
            confirm = Integer.parseInt(input.nextLine());
            if (confirm == 1) {
                System.out.println("As opções são:\n→ Cheque pelos correios\n→ Cheque em mãos\n→ Depósito em conta bancária (default).\n");
    ...
    private static void payEmployee(String[][] employees, double forPay, int i) {
        System.out.printf("O funcionário %s de ID [%s] recebeu R$ %.2f via %s.\n", employees[i][1],
    ...
    private static void auxPayRoll(String[][] employees, int i, int[][] calendary, int day, int month, String[] paySchedule) {
        ...
        if(employees[i][2].equals("2"))
            employees[i][10] = employees[i][3];
    }
